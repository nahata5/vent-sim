...
(0.3 s) pause; compliance is C = Vti/(Pplat − PEEP) measured at the end of that pause, and the initial ΔP
is Vt/C. Every later breath is PC at PEEP + ΔP, time-cycled; at each breath start the regulator corrects
...
`vt` (both the `applySettings` and `commitPending` paths) forces a new test breath, but a PEEP change does
not.
...
left for the step formula to divide by). A third fix-round change is bookkeeping, not physics:
`prvcRegulate` now runs at every breath start in every mode and clears `prvc-limit` when the mode is not
PRVC, since the alarm previously survived a mode change.
