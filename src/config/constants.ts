...
export const CONSTANTS = {
  ...
  DET_IE_PDEF: c(0.45, 'cmH2O', 'Brief 1 §3.1 Chen 2008: Paw deflection ≥ 0.45 cmH2O (sens 93.3%, spec 92.9%)', 'V'),
  DET_IE_FDEF_WITH_PDEF: c(3.0, 'L/min', 'Flow deflection (above the extrapolated decay) accepted when a concurrent smoothed Paw dip ≥ DET_IE_PDEF_SMOOTH confirms it; weak COPD efforts against high R give 3–7 L/min [M, tuned on the tuning grid]', 'M'),
  DET_IE_PDEF_SMOOTH: c(0.3, 'cmH2O', 'Paw dip on a 0.1 s moving average (noise RMS 0.05) confirming a small flow deflection; Chen 2008 Pdef 0.45 was read on raw traces [M]', 'M'),
  ...
  DET_PREM_NOTCH_PDEF: c(0.25, 'cmH2O', 'The premature-cycling notch must come with a Paw dip (the effort pulls against the exhalation valve); cardiac flow oscillation moves Paw < 0.15 [M]', 'M'),
  DET_AT_PAW_DIP: c(0.5, 'cmH2O', 'Brief 1 §3.2: auto-trigger = triggered breath with no Paw dip ≥ 0.5 cmH2O before the trigger', 'L'),
  ...
  DET_DC_PAW_RISE: c(2.0, 'cmH2O', 'Brief 1 §3.7: end-inspiratory Paw rise > 2 cmH2O above target in the last third [heuristic]', 'L'),
  DET_DC_SHOULDER_TAIL: c(0.4, 's', 'Time from a flow "shoulder" (abrupt steepening of the decay) to cycle-off that marks delayed cycling [M]', 'M'),
  ...
  DET_AT_LEAK_FLOW: c(0.5, 'L/min', 'Auto-trigger (leak): end-expiratory flow baseline sitting above zero by this much before the trigger [M]', 'M'),
