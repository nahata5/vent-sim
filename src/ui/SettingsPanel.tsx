...
const FIELDS: FieldDef[] = [
  ...
  { key: 'vt', label: 'Tidal volume', unit: 'mL', min: 100, max: 1200, step: 10, modes: ['VC-AC'] },
  { key: 'rr', label: 'Rate', unit: '/min', min: 4, max: 60, step: 1, modes: ['VC-AC', 'PC-AC'] },
  { key: 'peakFlow', label: 'Peak flow', unit: 'L/min', min: 10, max: 120, step: 5, modes: ['VC-AC'] },
  { key: 'pause', label: 'Insp. pause', unit: 's', min: 0, max: 2, step: 0.1, modes: ['VC-AC'] },
  { key: 'pinsp', label: 'Pinsp above PEEP', unit: 'cmH2O', min: 0, max: 40, step: 1, modes: ['PC-AC'] },
  { key: 'ti', label: 'Ti', unit: 's', min: 0.2, max: 3, step: 0.1, modes: ['PC-AC'] },
  { key: 'ps', label: 'Pressure support', unit: 'cmH2O', min: 0, max: 40, step: 1, modes: ['PSV'] },
  { key: 'ets', label: 'ETS (cycle-off)', unit: '% peak', min: 5, max: 80, step: 5, modes: ['PSV'], toDisplay: (v) => Math.round(v * 100), fromDisplay: (v) => v / 100 },
  { key: 'tiMax', label: 'Ti max', unit: 's', min: 0.5, max: 4, step: 0.1, modes: ['PSV'] },
  { key: 'riseTime', label: 'Rise time', unit: 's', min: 0, max: 0.4, step: 0.05, modes: ['PC-AC', 'PSV'] },
...
type Draft = Partial<Pick<VentSettings, NumKey>> & { mode?: Mode; triggerType?: VentSettings['triggerType']; flowPattern?: VentSettings['flowPattern'] };
...
export function SettingsPanel({ ctl, settings, pendingOnVent }: Props) {
  ...
  const visible = FIELDS.filter((f) => !f.modes || f.modes.includes(mode)).filter((f) =>
    f.key === 'flowTrigger' ? triggerType === 'flow' : f.key === 'pressureTrigger' ? triggerType === 'pressure' : true,
  );
